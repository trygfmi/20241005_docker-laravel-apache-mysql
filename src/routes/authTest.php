...
use App\Http\Controllers\Preview\PreviewRouteAuthTestController;
Route::get('create-auth-test-preview-top', [PreviewRouteAuthTestController::class, 'createIndex'])
->name('create-auth-test-preview-top-index');
Route::post('create-auth-test-preview-top', [PreviewRouteAuthTestController::class, 'create'])
->name('create-auth-test-preview-top');
...
->name('auth-test-name-email');
// nameとemailカラムがusersテーブルにあるか確認して、存在していたらログインして、nameとemail、userのクラスパス、ユーザー情報を表示します
Route::post('/auth-test-name-email', [AuthTestController::class, 'authLoginNameEmail'])
->name('auth-test-name-email');
...
Route::post('delete-user', [AuthTestController::class, 'deleteUser'])
->name('delete-user');
...
->name('auth-test-create-index');
// inputに入力された値で新規ユーザーを作成し、セッション経由でname,email,passwordを表示します
Route::post('auth-test-create', [AuthTestController::class, 'authTestCreate'])
->name('auth-test-create');